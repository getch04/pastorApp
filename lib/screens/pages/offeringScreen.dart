...
class _OfferingScreenItemState extends State<OfferingScreenItem> {
  ...
  void _onPaymentSuccess(
    ...
    bool saved = await _saveDonation(name, email, amount);
    if (saved) {
  ...
  void _onPaymentCancel() {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Payment was cancelled.')));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Image.asset(Img.get('new/Logo.png'), height: 40),
        centerTitle: true,
      ),
